...
const Card = ({ car }: Props) => {
  ...
  return (
    <div className='border border-zinc-200 shadow rounded hover:shodow-lg p-4 md:p-6'>
      <div>
      ...
      <div className='max-md:flex max-md:justify-between max-md:gap-10'>
        ...
        {arr.map((i,key) =>(
          <div className='text-zinc-500 flex justify-center items-center gap-2 text-lg' key={key}>
            <p>{i.icon}</p>
            <p>{i.value}</p>
          </div>
        ))}
